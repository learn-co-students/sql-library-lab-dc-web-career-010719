def select_books_titles_and_years_in_first_series_order_by_year
  "SELECT books.title, books.year FROM books JOIN series ON series.id = books.series_id WHERE series.id = (SELECT MIN(series.id) FROM series) ORDER BY books.year"
end
...
def select_value_and_count_of_most_prolific_species
  "SELECT species, COUNT(*) AS most FROM characters GROUP BY species ORDER BY COUNT(species) DESC LIMIT 1"
end
...
def select_series_title_with_most_human_characters
  # "SELECT series.title JOIN characters ON series.id = characters.series_id WHERE characters.species = 'human' GROUP BY COUNT(*) ORDER BY COUNT(*) LIMIT 1"
